...
export class AnalyticsService {
  ...
  static async getLowStockByBranch() {
    const lowStockItems = await AppDataSource.getRepository(BranchProduct)
  ...
  static async getCentralStockHealth() {
    const centralStocks = await AppDataSource.getRepository(CentralStock)
    ...
    const lowStock = centralStocks.filter(
      (cs) => Number(cs.quantity) < LOW_STOCK_MIN
    )
    const outOfStock = centralStocks.filter((cs) => Number(cs.quantity) === 0)
    const healthy = centralStocks.filter(
      (cs) => Number(cs.quantity) >= LOW_STOCK_MIN
    )
  ...
  static async getPendingRequests() {
    const pendingCount = await AppDataSource.getRepository(StockRequest)
      .createQueryBuilder("sr")
      .where("sr.status = :status", { status: "PENDING" })
      .getCount()

    const pendingRequests = await AppDataSource.getRepository(StockRequest)
      .createQueryBuilder("sr")
  ...
  static async getProductStatus() {
    const activeCount = await AppDataSource.getRepository(Product)
      .createQueryBuilder("p")
      .where("p.isActive = :isActive", { isActive: true })
      .getCount()

    const inactiveCount = await AppDataSource.getRepository(Product)
      .createQueryBuilder("p")
    ...
    return {
      ...
      activePercentage: (
        (activeCount / (activeCount + inactiveCount)) *
        100
      ).toFixed(2),
    }
  }
  ...
  static async getProductMovement() {
    ...
    const sorted = Object.values(productMovementCount).sort(
      (a: any, b: any) => b.count - a.count
    )

    return {
      highMoving: sorted.slice(0, 10).map((item: any) => ({
        productId: item.product.id,
        productName: item.product.name,
        sku: item.product.sku,
  ...
  static async getBranchWiseMovement() {
    ...
    return {
      branches: Object.values(branchMovements).sort(
        (a: any, b: any) => b.movementCount - a.movementCount
      ),
    }
  }
  ...
  static async getProductAging() {
    ...
    const agingData = await Promise.all(
      products.map(async (product) => {
        const lastMovement = await AppDataSource.getRepository(StockMovement)
          .createQueryBuilder("sm")
        ...
        const daysSinceLastMovement = lastMovement
          ? Math.floor(
              (Date.now() - lastMovement.createdAt.getTime()) /
                (1000 * 60 * 60 * 24)
            )
          : null
  ...
  static async getBranchRequestFrequency() {
    const requests = await AppDataSource.getRepository(StockRequest)
      .createQueryBuilder("sr")
      .leftJoinAndSelect("sr.branch", "branch")
      .getMany()
    ...
    return {
      branches: Object.values(branchRequestCount).sort(
        (a: any, b: any) => b.requestCount - a.requestCount
      ),
    }
  }

  static async getStockHoldingDuration() {
    const branchProducts = await AppDataSource.getRepository(BranchProduct)
      .createQueryBuilder("bp")
      .leftJoinAndSelect("bp.branch", "branch")
      .leftJoinAndSelect("bp.product", "product")
      .where("bp.quantity > 0")
  ...
  static async getRequestStatusBreakdown() {
    ...
    const breakdown = await Promise.all(
      statuses.map(async (status) => {
        const count = await AppDataSource.getRepository(StockRequest)
          .createQueryBuilder("sr")
  ...
  static async getPartialApprovals() {
    ...
    return {
      ...
      percentage: ((partialApprovals.length / requests.length) * 100).toFixed(
        2
      ),
    }
  }
  ...
  static async getUserActivity() {
    ...
    return {
      users: Object.values(userActivity).sort(
        (a: any, b: any) => b.requestCount - a.requestCount
      ),
    }
  }
  ...
  static async getBranchOverview(branchId: string) {
    const branchProducts = await AppDataSource.getRepository(BranchProduct)
      .createQueryBuilder("bp")
    ...
    const lowStockCount = branchProducts.filter(
      (bp) => Number(bp.quantity) <= LOW_STOCK_MIN
    ).length
  ...
  static async getBranchLowStock(branchId: string) {
    const lowStockItems = await AppDataSource.getRepository(BranchProduct)
      .createQueryBuilder("bp")
      .leftJoinAndSelect("bp.product", "product")
      .where("bp.branch.id = :branchId", { branchId })
  ...
  static async getBranchRecentDispatched(branchId: string) {
    ...
    const recentDispatched = requests.flatMap((req) =>
      req.items.map((item) => ({
        productName: item.product.name,
