...
exports.generateStockRequestReceipt = generateStockRequestReceipt;
exports.generateRequestCreatedReceipt = generateRequestCreatedReceipt;
exports.generateApprovalReceipt = generateApprovalReceipt;
exports.generateDispatchReceipt = generateDispatchReceipt;
exports.generateReceiveReceipt = generateReceiveReceipt;
// src/utils/pdfReceipt.ts
const pdfkit_1 = __importDefault(require("pdfkit"));
const StockRequest_1 = require("../entities/StockRequest");
// Color schemes for different statuses
const STATUS_COLORS = {
    [StockRequest_1.StockRequestStatus.PENDING]: { primary: "#F59E0B", bg: "#FEF3C7", text: "#92400E" },
    [StockRequest_1.StockRequestStatus.APPROVED]: { primary: "#3B82F6", bg: "#DBEAFE", text: "#1E40AF" },
    [StockRequest_1.StockRequestStatus.DISPATCHED]: { primary: "#8B5CF6", bg: "#EDE9FE", text: "#5B21B6" },
    [StockRequest_1.StockRequestStatus.RECEIVED]: { primary: "#10B981", bg: "#D1FAE5", text: "#065F46" },
    [StockRequest_1.StockRequestStatus.REJECTED]: { primary: "#EF4444", bg: "#FEE2E2", text: "#991B1B" },
};
...
function drawStatusBadge(doc, status, x, y, colors) {
...
function drawItemsTable(doc, items, startY, status, colors) {
    const tableLeft = 50;
    const tableWidth = doc.page.width - 100;
    const isReceived = status === StockRequest_1.StockRequestStatus.RECEIVED;
    const isApprovedOrDispatched = [
        StockRequest_1.StockRequestStatus.APPROVED,
        StockRequest_1.StockRequestStatus.DISPATCHED,
    ].includes(status);
    let colWidths;
    let headers;
    if (isReceived) {
...
function generateStockRequestReceipt(request, options) {
    ...
    const showApprovedColumn = [
        StockRequest_1.StockRequestStatus.APPROVED,
        StockRequest_1.StockRequestStatus.DISPATCHED,
        StockRequest_1.StockRequestStatus.RECEIVED,
    ...
    if (showApprovedColumn && request.items?.length > 0) {
        ...
        if (request.status === StockRequest_1.StockRequestStatus.RECEIVED) {
...
function generateRequestCreatedReceipt(request, requesterName) {
...
function generateApprovalReceipt(request, approverName) {
    return generateStockRequestReceipt(request, {
...
function generateDispatchReceipt(request, dispatcherName) {
    return generateStockRequestReceipt(request, {
        title: "DISPATCH NOTE",
...
function generateReceiveReceipt(request, receiverName) {
    return generateStockRequestReceipt(request, {
