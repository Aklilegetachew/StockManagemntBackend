...
class ReportService {
    ...
    static async getStockMovements(filters) {
        ...
        const additions = movements
            .filter((m) => [StockMovement_1.StockMovementType.ADDITION, StockMovement_1.StockMovementType.TRANSFER_IN].includes(m.type))
            .reduce((sum, m) => sum + Number(m.quantity), 0);
        const deductions = movements
            .filter((m) => [StockMovement_1.StockMovementType.DEDUCTION, StockMovement_1.StockMovementType.SALE, StockMovement_1.StockMovementType.TRANSFER_OUT].includes(m.type))
            .reduce((sum, m) => sum + Number(m.quantity), 0);
        return {
     ...
     */
    static async getRequestedItems(filters) {
        ...
        const totalRequested = requests.reduce((sum, r) => sum + r.items.reduce((itemSum, item) => itemSum + Number(item.requestedQuantity), 0), 0);
        const totalApproved = requests.reduce((sum, r) => sum + r.items.reduce((itemSum, item) => itemSum + Number(item.approvedQuantity || 0), 0), 0);
        return {
...
ReportService.branchProductRepo = data_source_1.AppDataSource.getRepository(BranchProduct_1.BranchProduct);
ReportService.centralStockRepo = data_source_1.AppDataSource.getRepository(CentralStock_1.CentralStock);
ReportService.stockMovementRepo = data_source_1.AppDataSource.getRepository(StockMovement_1.StockMovement);
ReportService.stockRequestRepo = data_source_1.AppDataSource.getRepository(StockRequest_1.StockRequest);
ReportService.salesReportRepo = data_source_1.AppDataSource.getRepository(SalesReport_1.SalesReport);
ReportService.salesReportItemRepo = data_source_1.AppDataSource.getRepository(SalesReportItem_1.SalesReportItem);
ReportService.branchRepo = data_source_1.AppDataSource.getRepository(branches_1.Branch);
ReportService.productRepo = data_source_1.AppDataSource.getRepository(Product_1.Product);
